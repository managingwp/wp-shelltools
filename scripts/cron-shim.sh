...
CRON_OUTPUT=$($WP_CLI cron event run --due-now --path=$WP_ROOT 2>&1)
...
if [[ $LOG_TO_SYSLOG == 1 ]]; then
  logger -t "wordpress-crons" "Cron job completed in $TIME_SPENT seconds with $CPU_USAGE% CPU usage. Output: $CRON_OUTPUT"
else
  # Log to file in the WordPress install directory
  echo "$(date +"%Y-%m-%d %H:%M:%S") - WordPress crons completed in $TIME_SPENT seconds with $CPU_USAGE% CPU usage. Output: $CRON_OUTPUT" >> $LOG_FILE
fi
